...
var want_full_html = $('#full_html').hasClass('included'),
		...
		part_html = "<div id=\"fav\">Your Favourite HTML,CSS,JS Playground!</div>",
...
function remove_resources(doc, type) {
	...
	if( typeof doc == type ) {
		...
		html = html.replace(/[\n][\t]*<link class="csscreations"[^\n]* /g, ''); //remove space from regex end if want to use
...
var timeline_ulp; //update_live_preview for timeline page
...
code_mirror_html = CodeMirror.fromTextArea( $('#html textarea')[0], {
	...
	onChange: function(instance, changes) {
		...
		if($save_timeline !== true && ($new_content != $ohc && should_render()))
		{
			update_live_preview();
		}
		...
		{
			check_for_help("html", $new_content);
			timeline_ulp = setTimeout(update_live_preview, 500);
	...
	onCursorActivity: function(instance)
	{
		...
		//why is this here ? why update live preview on cursor movements at the end of the WT ? same with the other 2 editors below ?
		//deprecated/falsed on 3/10/13 for matrix rain entry
		else if($save_timeline !== true && $slide == $slides-1 && false) 
		{
			update_live_preview();
		}
	}, 
...
code_mirror_css = CodeMirror.fromTextArea( $('#css textarea')[0], {
	...
	onChange: function(instance, changes) {
		...
		if($save_timeline !== true && ($new_content != $occ && should_render()))
		{
			//if($slide == $slides-1) console.log("WT finished(CSS) onchange");
	...
	onCursorActivity: function(instance)
	{
		...
		{
			//if($slide == $slides-1) console.log("WT finished(CSS) cursor");
...
code_mirror_js = CodeMirror.fromTextArea( $('#js textarea')[0], {
	...
	onChange: function(instance, changes) {
		...
		if($save_timeline !== true && ($new_content != $ojc && should_render()))
		{
			update_live_preview();
		}
...
function check_for_help($editor, $content)
{
	if($editor == "html")
	{
		if($content.toLowerCase().indexOf("<!doctype") !== -1 && $auto_help_shown.doctype !== true)
		{
			customalert("Doctype not required", "You don't need to mention the doctype. Only write the HTML code which falls inside <body></body>");
			$auto_help_shown.doctype = true;
		}
		if($content.toLowerCase().indexOf("<html>") !== -1 && $auto_help_shown.html !== true)
		{
			customalert("<html> tags not required", "You don't need to mention <html> tags. Only write the HTML code which falls inside <body></body>");
			$auto_help_shown.html = true;
		}
		if($content.toLowerCase().indexOf("<body>") !== -1 && $auto_help_shown.body !== true)
		{
			customalert("<body> tags not required", "You don't need to mention <body> tags. Only write the HTML code which falls inside it");
			$auto_help_shown.body = true;
		}
		if($content.toLowerCase().indexOf("<head>") !== -1 && $auto_help_shown.head !== true)
		{
			customalert("<head> tags not required", "You don't need to mention <head> tags. Only write the HTML code which falls inside <body></body>");
			$auto_help_shown.head = true;
		}
		//<link href='http://fonts.googleapis.com/css?family=Alex+Brush' rel='stylesheet' type='text/css'>
	}
	if($editor == "css")
	{
		///* Font by Bennett Feely http://codepen.io/bennettfeely/pen/GCAKJ */ @font-face { font-family:icon; src: url('http://bennettfeely.com/fonts/icons.woff'); }
	}
}
...
// HTML, CSS, JS TABS
$('.code_html, .code_css, .code_js').click( function() {
	//return codemirror_expand( $(this).attr('data-type') );
	$editor = $(this).attr('data-type');
	if($(this).hasClass("enabled"))
	{
		$("#"+$editor).removeClass("enabled").hide();
		$(this).removeClass("enabled");
	}
	else
	{
		$("#"+$editor).addClass("enabled").show();
...
update_live_preview = function() {
	...
	if(typeof already_injecting == "undefined")
		already_injecting = false;

	if(!already_injecting || $slide == $slides - 1)
	{
		// if($slide == $slides - 1)
		// 	console.log("last render - "+$slide+" of "+$slides);

		$logs.updates++;
	
		code_mirror_save();
	
		if(typeof active_iframe == "undefined")
			active_iframe = "two";
	
		iframe_one = $("#output iframe.one");
		iframe_two = $("#output iframe.two");
	
		if(!iframe_two.length)
			iframe_one.replaceWith('<iframe id="docCanvas" class="one" src="about:blank"></iframe>');
		else if(active_iframe == "two")
			iframe_one.replaceWith('<iframe id="docCanvas" class="one" src="about:blank" style="visibility: hidden;"></iframe>');
		else if(active_iframe == "one")
			iframe_two.replaceWith('<iframe id="docCanvas2" class="two" src="about:blank" style="visibility: hidden;"></iframe>');
		...
		if ( css_code ) prepared_source = prepared_source.replace('</head>', PrefixFree.prefixCSS(css, true) + '</head>');
		if ( js_code ) prepared_source = prepared_source.replace('</body>', js + '</body>');
	
		ocontent = $('#docCanvas').contents();
		ocontent_two = $('#docCanvas2').contents();
	
		setTimeout(inject_iframe_content, 0);
	}
	else
	{
		//console.log("sorry, already injecting");
	}
};

function inject_iframe_content()
{
	already_injecting = true;
	if(active_iframe == "two")
	{
		ocontent = $('#docCanvas').contents(); ocontent[0].open();	ocontent[0].write(prepared_source); ocontent[0].close();
		
		if(iframe_two.length) 
			swapper = setTimeout(swap_iframes, 300);
		else
			already_injecting = false;
	}
	else if(active_iframe == "one")
	{
		ocontent_two = $('#docCanvas2').contents(); ocontent_two[0].open(); ocontent_two[0].write(prepared_source); ocontent_two[0].close();
	...
	old_prepared_source = prepared_source;
}

function swap_iframes()
{
	if(active_iframe == "two")
	{
		$("#output iframe.one").css({visibility: "visible"});
		$("#output iframe.two").css({visibility: "hidden"});
		active_iframe = "one";
	}
	else if(active_iframe == "one")
	{
		$("#output iframe.two").css({visibility: "visible"});
		$("#output iframe.one").css({visibility: "hidden"});
		active_iframe = "two";
	}
	already_injecting = false;
}
...
//console.log($('#docCanvas').readyState);
//console.log($('#docCanvas').contents()[0].documentElement.innerHTML);
$('#docCanvas').on('load', function() {	// Chrome Iframe prob fix
	//console.log('Load Event Triggered');
	if ( $('#docCanvas').contents().find('body *').length < 1 ) update_live_preview();
	$('#docCanvas').off('load'); // Important, or else messes with other loads
});
//reset_resources = false;
var iframe_links_fixed = false;

var fix_iframe_links = function() {
	if( window.history && history.pushState ) {
		// Iframe Refresh

		try {
			$('#docCanvas')[0].contentWindow.history.replaceState(null, null, location.href);
		}
		catch(error) {
			output_reload();
		}
	...
	output_reload(); // TERRIBLE FIX
	iframe_links_fixed = true;
};
...
if ( !iframe_links_fixed ) fix_iframe_links();

// ADDING TABS
$('#html textarea, #css textarea, #js textarea').live('keydown', function(e) {
	if( e.keyCode == 9 ) {
		var $this = $(this);
...
/*
	RESOURCES
*/
/* Entire resources JS deprecated on 26052013
